...
func TestToggleScopeInstalledPlugin(t *testing.T) {
	...
	m.toggleScope()
	if op, ok := m.pendingOps["test@marketplace"]; !ok || op.Type != OpMigrate || op.Scope != claude.ScopeProject {
		t.Errorf("after second toggle = %v, want OpMigrate with project", op)
	}
	// Also verify original scope is preserved
	if op := m.pendingOps["test@marketplace"]; op.OriginalScope != claude.ScopeLocal {
